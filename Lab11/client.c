...
int main(int argc, char** argv) {
    ...
    // ...
        ...
        while (!should_close) {
            if (scanf("Input type: %ms", &request_type_input_buffer) == 1) {
                request_message_t message;
                ...
                } else if (strncpy(request_type_input_buffer, "2ALL", 4) == 0) {
                    message.request_type = TOALL;
                    scanf("Input message: %s",message.payload.to_all);
                    send(socket_fd, &message, sizeof(message), MSG_DONTWAIT);
                } else if (strncpy(request_type_input_buffer, "2ONE", 4) == 0) {
                    message.request_type = TOONE;
                    scanf("Input client: %s",message.payload.to_one.target_client);
                    scanf("Input message: %s",message.payload.to_one.message);
                    send(socket_fd, &message, sizeof(message), MSG_DONTWAIT);
                } else {
